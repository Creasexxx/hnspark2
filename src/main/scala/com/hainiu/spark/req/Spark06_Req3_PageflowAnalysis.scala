...
object Spark06_Req3_PageflowAnalysis {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val actionRDD = sc.textFile("D:\\temp\\user_visit_action.txt")
        ...
        val okflowIds: List[(Long, Long)] = ids.zip(ids.tail)//ids.tail：不包含第一个

        // TODO 计算分母
        val pageidToCountMap: Map[Long, Long] = actionDataRDD.filter(
            action => {
                //ids.init:不包括最后一个（这里的考虑是：最后不需要统计）
